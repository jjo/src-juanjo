 ...
 * Date: Wed Dec 20 09:26:29 ART 2006
 ...
 # Explanation:
   pthread_mutex is a mutual exclusion lock usually used un multi-threaded applications.
   This program shows a mutex usage that SURVIVES de timelife of a process (hence can be
   used between processes) by making a shared-memory storage for the pthread_mutex_t object.
 ...
 # Usage:
 	...
 	./mproc-mutex foo i   #initialize "foo" mutex, will appear as /dev/shm/foo
...
int main(int argc, char **argv)
{
	int fd;
	pthread_mutex_t *p;
	if (argc!=3) {fprintf(stderr, "usage: %s /mutex_file_name {i|l|u}\n", argv[0]);exit(EXIT_FAILURE);}

	ERR_EXIT( fd=shm_open(argv[1], O_RDWR|O_CREAT, 0666) );

	if (*argv[2]=='i') 
		ERR_EXIT( ftruncate(fd, sizeof(pthread_mutex_t)) ); /* expand the shm file to required size */
