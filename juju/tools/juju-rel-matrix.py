...
def print_relations_matrix(juju_services, args):
    ...
    print '{}X{}'.format(prefix, args.separator), \
    ...
    for service_row in sorted(juju_services.keys()):
        ...
        for _service_col, service_col_dict in sorted(juju_services.iteritems()):
            rel_output = []
...
for filename in cmd_args.files:
    ...
    if not juju_status:
        print >> sys.stderr, "ERROR: {} doesn't parse as YAML", format(filename)
        continue
